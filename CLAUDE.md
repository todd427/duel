...
- **Persistence (all `localStorage`):** theme (`duel_theme`); web toggle (`duel_web`); per-server context toggles (`duel_mcp_<id>_on`); system prompts + model choices + max-tokens (`duel_settings`, via `saveSettings()` over `PERSIST_IDS`). No secrets are stored client-side. Four themes live as CSS via `data-t` attributes and `setTheme()`.
